...
function renderArray(arr, curData) {
  ...
  if (isObject(type)) {
    curData.forEach(e => {
      e = renderObject(arr.items.properties)
    })

  }
  if (isArray(type)) {
    curData.forEach(e => {
      e = renderObject(arr.items)
    })
...
class EnhancedTable extends React.Component {
  ...
  componentDidMount() {
    ...
    fetch(url).then(res => res.json()).then((result) => {
      ...
      this.timer = setTimeout(() => this.setState({isLoaded: true, items: result.items}), 900);
    }, (error) => {
  ...
  generateTableCell = (a, b, c) => {
    ...
    if (prop.type !== "array" && prop.type !== "object") {
      ...
      return <TableCell numeric={false} key={c} style={{
          color: error.trim() !== ''
            ? 'red'
            : ''
        }}>{this.state.data[a.id].properties[b].value}
        {
          error.trim() !== ''
            ? <Warning/>
            : ''
  ...
  archive = _ => {
    ...
    this.setState({previousData: prevData})
