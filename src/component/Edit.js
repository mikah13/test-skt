...
class Edit extends React.Component {
  ...
  generateTextField = (a, idx, s, parents) => {
    ...
    return <div style={{
        paddingLeft: margin
      }}><TextField error {helperText.trim() === ""?false:true} placeholder={desc} key={`tf-${idx}`} id={prop} required={this.props.schema.required.indexOf(prop) !== -1
        ? true
        : false} label={prop} type="" value={value} helperText={helperText}
      //test
      margin="normal" style={{
        width: '50%',
        marginLeft: '25%'
      }} onChange={(e) => this.handleInputChange(e, a, parents)}/></div>
  }
  ...
  generateObject = (a, idx, s, parents) => {
    ...
    return propArray.map((b, i) => {
      if (i === 0) {
        return <div key={`objDiv-${i}`}>
          <div key={`textFieldObjDiv-${i}`} style={{
              paddingLeft: (parents.length * 100) + "px"
            }}><TextField key={`textFieldObj-${i}`} InputProps={{
            readOnly: true
          }} margin="normal" style={{
            width: '70px',
            marginLeft: '25%'
              ...
              isChildrenOfArray
                ? <DeleteElementButton parents={newParents} delete="delete" ={(p)=>{this.deleteArrayElement(p)}}/>
                : null
            }
          </div>{this.generateInput(b, i, a, newParents)}</div>
      }
  ...
  generateArray = (a, idx, s, parents) => {
    ...
    if (propArray.length === 0) {
      return <div style={{
          paddingLeft: (parents.length * 100) + "px"
        }}><TextField defaultValue={a.title} InputProps={{
          readOnly: true
        }} margin="normal" style={{
          width: '70px',
          marginLeft: '25%'
        }}/>
        <AddElementButton parents={newParents} add={(p) => {
            this.addArrayElement(p)
          }}/></div>
    }
    return propArray.map((b, i) => {
      if (i === 0) {
        return <div key={`objDiv-${i}`}>
          <div key={`textFieldArrayDiv-${i}`} style={{
              paddingLeft: (parents.length * 100) + "px"
            }}><TextField key={`textFieldArray-${i}`} defaultValue={a.title} InputProps={{
            readOnly: true
          }} margin="normal" style={{
            width: '70px',
            marginLeft: '25%'
